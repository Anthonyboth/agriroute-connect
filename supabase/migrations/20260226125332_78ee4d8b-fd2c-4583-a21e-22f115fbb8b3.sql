UPDATE public.service_requests SET status = 'CANCELLED', updated_at = now() WHERE status = 'OPEN' AND service_type IN ('ENTREGA_PACOTES','TRANSPORTE_PET','MUDANCA','MUDANCA_RESIDENCIAL','MUDANCA_COMERCIAL','FRETE_MOTO','GUINCHO','CARGA','FRETE_URBANO','CARREGAMENTO_DESCARREGAMENTO_URB');

CREATE OR REPLACE FUNCTION public.get_unified_service_feed(p_profile_id uuid, p_debug boolean DEFAULT false) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public', 'extensions' AS $function$
DECLARE
  v_role text;
...
BEGIN
  ...
  IF v_role = 'PRESTADOR_SERVICOS' THEN
    ...
    viewer_cities AS (
      SELECT DISTINCT uc.city_id FROM public.user_cities uc
    ...
    eligible_strict AS (SELECT * FROM service_eval WHERE type_match AND city_match),
    strict_count AS (SELECT count(*)::int AS cnt FROM eligible_strict),
    chosen AS (
      SELECT * FROM eligible_strict WHERE (SELECT cnt FROM strict_count) > 0
      UNION ALL
      SELECT * FROM service_eval WHERE (SELECT cnt FROM strict_count) = 0
    ),
    chosen_limited AS (SELECT * FROM chosen ORDER BY created_at DESC LIMIT 50)
    SELECT
      ...
      jsonb_build_object(
        ...
        'excluded',coalesce((SELECT jsonb_agg(jsonb_build_object('item_type','SERVICE','item_id',x.id,'service_type',x.canonical_service_type,'reason',CASE WHEN NOT x.type_match THEN 'service_type' WHEN NOT x.city_match THEN 'location' ELSE 'unknown' END)) FROM service_eval x WHERE NOT (x.type_match AND x.city_match)),'[]'::jsonb)
      ),
...
COMMENT ON FUNCTION public.get_unified_service_feed(uuid, boolean) IS 'Feed de serviços para PRESTADOR. Exclui tipos freight (ENTREGA_PACOTES, TRANSPORTE_PET, MUDANCA, etc). Fallback controlado quando match estrito é zero.';