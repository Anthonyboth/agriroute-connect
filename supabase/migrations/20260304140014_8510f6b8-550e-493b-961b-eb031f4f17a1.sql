...
-- =====================================================
-- CORRIGIR auto_cancel_overdue_freights: CARGA usa 72h após pickup_date (não 48h)
-- Regra: fretes de CARGA vencem 72h após a data de coleta agendada
-- =====================================================
...
BEGIN
  ...
  -- CORRIGIDO: 72h após pickup_date (não 48h)
  -- =========================================================
  FOR v_freight_record IN
    SELECT 
      f.id,
  ...
  -- CORRIGIDO: CARGA usa 72h após pickup_date (não 48h)
  -- =========================================================
  FOR v_freight_record IN
    ...
    WHERE f.status IN ('OPEN', 'IN_NEGOTIATION')
      AND f.accepted_trucks = 0
      AND (
        (f.service_type = 'GUINCHO' AND f.created_at + INTERVAL '2 hours' < NOW())
        OR
...
GUINCHO: 2h | FRETE_MOTO: 4h | URBANOS: 24h | MUDANÇA: 48h | CARGA: 72h após pickup_date | SERVIÇOS sem data: 72h após criação.';
