-- Fix get_freights_for_driver RPC to resolve type mismatch

-- Drop any existing versions
DROP FUNCTION IF EXISTS public.get_freights_for_driver(uuid);

-- Recreate with correct urgency type cast
...
BEGIN
  ...
  SELECT
    ...
    f.delivery_date,
    f.price,
    COALESCE((f.urgency)::text, 'LOW') AS urgency,  -- Cast urgency enum to text
    (f.status)::text AS status,                      -- Cast status enum to text
    f.distance_km,
