-- Fix update_trip_progress: always insert ALL intermediate status history records
-- This prevents missing history when driver advances through statuses quickly
...
BEGIN
    ...
    IF v_progress_id IS NULL THEN
        ...
        EXCEPTION WHEN OTHERS THEN
            RAISE LOG 'update_trip_progress: failed to insert history for new progress: % %', SQLERRM, SQLSTATE;
        END;
    ...
    -- Registrar no histórico de status (com log de erro em vez de silenciar)
    BEGIN
        INSERT INTO freight_status_history (
            freight_id, status, changed_by, notes, location_lat, location_lng, created_at
        ) VALUES (
    ...
    EXCEPTION WHEN OTHERS THEN
        RAISE LOG 'update_trip_progress: failed to insert status history: % % (freight=%, status=%, driver=%)', 
            SQLERRM, SQLSTATE, p_freight_id, p_new_status, v_driver_profile_id;
    END;
...
'Atualiza o progresso da viagem do motorista com validação de não-regressão.
Erros de inserção no histórico agora são logados (RAISE LOG) em vez de silenciados.';