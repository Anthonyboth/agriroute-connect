-- Atualiza a função para respeitar cidades/estados configurados no perfil do prestador
...
BEGIN
  ...
  WHERE (r.provider_id = provider_profile_id OR r.status = 'PENDING')
    ...
    AND (
      -- Sem regiões configuradas: não filtra por região
      ((provider_cities IS NULL OR array_length(provider_cities, 1) IS NULL)
        AND (provider_states IS NULL OR array_length(provider_states, 1) IS NULL))
      OR (
        -- Coincidência por cidade
        array_length(provider_cities, 1) IS NOT NULL AND EXISTS (
          SELECT 1 FROM unnest(provider_cities) AS city
          WHERE COALESCE(r.location_address, '') ILIKE '%' || city || '%'
        )
      )
      OR (
        -- Coincidência por estado (sigla)
        array_length(provider_states, 1) IS NOT NULL AND EXISTS (
          SELECT 1 FROM unnest(provider_states) AS st
          WHERE COALESCE(r.location_address, '') ILIKE '%' || st || '%'
        )
      )
    )
  ...
  WHERE gr.request_type = 'SERVICE'
    ...
    AND (
      ...
      ((provider_cities IS NULL OR array_length(provider_cities, 1) IS NULL)
        AND (provider_states IS NULL OR array_length(provider_states, 1) IS NULL))
      OR (
        -- Coincidência por cidade no endereço de origem informado no payload
        array_length(provider_cities, 1) IS NOT NULL AND EXISTS (
          SELECT 1 FROM unnest(provider_cities) AS city
          WHERE COALESCE(gr.payload->>'origin_address', '') ILIKE '%' || city || '%'
        )
      )
      OR (
        -- Coincidência por estado no endereço de origem
        array_length(provider_states, 1) IS NOT NULL AND EXISTS (
          SELECT 1 FROM unnest(provider_states) AS st
          WHERE COALESCE(gr.payload->>'origin_address', '') ILIKE '%' || st || '%'
        )
      )
    )
