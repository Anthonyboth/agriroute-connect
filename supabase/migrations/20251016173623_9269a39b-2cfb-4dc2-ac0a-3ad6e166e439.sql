-- Fix NOT_FREIGHT_DRIVER: Allow drivers with active assignment to update freight status
...
BEGIN
  ...
  -- Authorize if direct driver OR has active assignment
  ...
  -- Effective driver for history
  v_effective_driver := COALESCE(v_freight.driver_id, v_profile_id);

  -- Update freight (set driver_id if was null and driver has assignment)
  UPDATE public.freights
  SET status = p_new_status,
      driver_id = CASE 
        WHEN driver_id IS NULL AND v_is_assigned THEN v_profile_id
        ELSE driver_id
      END,
      updated_at = now(),
      metadata = CASE 
