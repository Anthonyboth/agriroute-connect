-- Fix delivery reporting reliability: increase lock timeout + make status updates idempotent
-- This targets the "canceling statement due to lock timeout" and repeated-click/queue retries causing trigger errors.
...
BEGIN
  ...
  -- ✅ Idempotency: if the status is already set, do nothing (prevents trigger side effects + lock churn)
