-- Fix: get_authoritative_feed deve retornar service_requests para MOTORISTA/MOTORISTA_AFILIADO/TRANSPORTADORA
-- quando possuem tipos urbanos (GUINCHO, MUDANCA, FRETE_MOTO, ENTREGA_PACOTES, TRANSPORTE_PET).
-- Antes, serviços só eram retornados para PRESTADOR_SERVICOS.
-- Também adiciona contexto de debug (viewer_cities, viewer_types, viewer_profile_id) para instrumentação anti-sumiço.
...
BEGIN
  ...
  eligible AS (SELECT * FROM evaluated e WHERE e.type_match AND e.location_match AND e.status_match ORDER BY e.created_at DESC),
  debug_rows AS (
    SELECT e.id, e.canonical_service_type,
      CASE WHEN NOT e.status_match THEN 'status' WHEN NOT e.type_match THEN 'service_type' WHEN NOT e.location_match THEN 'location' ELSE NULL END AS reason
    FROM evaluated e
  )
  SELECT
  ...
  -- Retorna para TODOS os roles quando possuem tipos compatíveis
  IF v_role = 'PRESTADOR_SERVICOS' THEN
    v_has_urban_types := true;
  ELSE
    -- Checar se o viewer tem tipos urbanos
  ...
  IF v_has_urban_types THEN
    ...
    service_eligible AS (SELECT * FROM service_eval WHERE type_match AND city_match ORDER BY created_at DESC)
    SELECT
