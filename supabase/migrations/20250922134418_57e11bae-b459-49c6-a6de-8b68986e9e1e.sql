      ...
      SELECT UNNEST(
        CASE 
          ...
          THEN ARRAY(
            SELECT DISTINCT 
              CASE 
                WHEN v.type = 'RODOTREM' THEN 'rodotrem'::service_category
                WHEN v.type = 'CARRETA' THEN 'carreta'::service_category
                WHEN v.type = 'TRUCK' THEN 'truck'::service_category
                WHEN v.type = 'TOCO' THEN 'truck'::service_category
                WHEN v.type = 'VUC' THEN 'vuc'::service_category
                WHEN v.type = 'PICKUP' THEN 'pickup'::service_category
                WHEN v.type = 'PRANCHA' THEN 'prancha'::service_category
                WHEN v.type = 'GUINCHO' THEN 'guincho'::service_category
          ...
          WHEN EXISTS(SELECT 1 FROM profiles WHERE user_id = auth.uid() AND role = 'PRESTADOR_SERVICO')
...
-- Insert all plans according to the table provided
...
-- Caminhão prancha
('prancha', 'free', 'Plano Grátis - Prancha', 0, 5.00, '["Acesso básico à plataforma", "5% de taxa sobre fretes"]'),
('prancha', 'essential', 'Plano Essencial - Prancha', 129.00, 2.00, '["Suporte prioritário", "R$ 129/mês + 2% sobre fretes", "Dashboard avançado"]'),
('prancha', 'professional', 'Plano Profissional - Prancha', 199.00, 0, '["Sem taxa sobre fretes", "R$ 199/mês fixo", "Suporte premium", "Relatórios completos"]'),

-- Guinchos
('guincho', 'free', 'Plano Grátis - Guincho', 0, 5.00, '["Acesso básico à plataforma", "5% de taxa sobre fretes"]'),
('guincho', 'essential', 'Plano Essencial - Guincho', 119.00, 2.00, '["Suporte prioritário", "R$ 119/mês + 2% sobre fretes", "Dashboard avançado"]'),
('guincho', 'professional', 'Plano Profissional - Guincho', 199.00, 0, '["Sem taxa sobre fretes", "R$ 199/mês fixo", "Suporte premium", "Relatórios completos"]'),
