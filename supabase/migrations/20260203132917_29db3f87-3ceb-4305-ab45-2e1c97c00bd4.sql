...
-- Corrigir função get_affiliated_driver_profile
-- Problema: Estava comparando tc.profile_id com auth.uid() (user_id)
-- Mas tc.profile_id aponta para profiles.id, não para auth.uid()
-- Solução: Buscar o profile_id do usuário logado e comparar
...
BEGIN
  -- ✅ CORREÇÃO: Buscar o profile.id do usuário logado (auth.uid() é user_id, não profile.id)
  ...
  -- Verificar se o usuário é dono da transportadora (comparando profile.id, não auth.uid())
  IF NOT EXISTS (
    SELECT 1 FROM transport_companies tc
    WHERE tc.id = p_company_id AND tc.profile_id = v_caller_profile_id
  ) THEN
...
-- Também corrigir a política RLS profiles_select_affiliated_drivers que tem o mesmo problema
...
CREATE POLICY "profiles_select_affiliated_drivers" ON public.profiles
  ...
  USING (
    ...
    id IN (
      ...
      -- ✅ CORREÇÃO: tc.profile_id aponta para profiles.id, não auth.uid()
      -- Precisamos buscar o profile.id do caller
      INNER JOIN profiles caller ON caller.user_id = auth.uid()
      WHERE tc.profile_id = caller.id
        AND cd.status IN ('ACTIVE', 'INACTIVE', 'PENDING')
    )
  );
