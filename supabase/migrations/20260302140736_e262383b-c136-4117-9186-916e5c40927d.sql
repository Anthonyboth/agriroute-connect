-- Make freight_status_history INSERT MANDATORY in update_trip_progress
-- If the history insert fails, the ENTIRE transaction rolls back.
-- This guarantees complete audit trail for every status transition.
...
BEGIN
    ...
    IF v_progress_id IS NULL THEN
        ...
        -- ✅ OBRIGATÓRIO: Registrar no histórico (SEM try/catch - falha = rollback)
    ...
    IF v_new_index < v_current_index THEN
        RETURN jsonb_build_object(
            ...
            'message', 'Não é permitido voltar de "' || v_current_status || '" para "' || p_new_status || '".',
    ...
    IF v_new_index > v_current_index + 1 THEN
        RETURN jsonb_build_object(
            ...
            'message', 'Não é permitido pular etapas. De "' || v_current_status || '" só pode ir para o próximo.',
            'current_status', v_current_status,
            'attempted_status', p_new_status,
    ...
    -- ✅ OBRIGATÓRIO: Registrar no histórico (SEM try/catch - falha = rollback total)
    INSERT INTO freight_status_history (
        freight_id, status, changed_by, notes, location_lat, location_lng, created_at
    ) VALUES (
        p_freight_id,
    ...
    -- Sincronizar com tabelas legadas (silenciosamente - estas SIM podem falhar)
    BEGIN
...
'Atualiza progresso da viagem com validação de não-regressão.
INSERT em freight_status_history é OBRIGATÓRIO - se falhar, toda a transação é revertida.
Isso garante trilha de auditoria completa para segurança e rastreabilidade.';