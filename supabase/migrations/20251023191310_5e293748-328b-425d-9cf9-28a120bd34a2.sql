-- Corrigir contagem de motoristas para incluir autônomos + afiliados (SEM transportadoras)
...
BEGIN
  ...
  SELECT 
    ...
    -- Contar motoristas autônomos + afiliados (SEM transportadoras)
    (SELECT COUNT(*) FROM public.profiles 
     WHERE role IN ('MOTORISTA', 'MOTORISTA_AFILIADO'))::bigint as motoristas,
    (SELECT COUNT(*) FROM public.profiles 
     WHERE role = 'PRESTADOR_SERVICOS' AND status = 'APPROVED')::bigint as prestadores,
    ...
    (SELECT COALESCE(AVG(rating), 0) FROM public.profiles 
     WHERE rating IS NOT NULL AND rating > 0) as avaliacao_media;
END;
$function$;