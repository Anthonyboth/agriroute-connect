...
BEGIN
  ...
  SELECT DISTINCT ON (f.id)
    ...
    f.service_type,
    f.created_at,
    -- Calculate distance from driver's city to freight origin
    ROUND(CAST(
      6371 * acos(
        GREATEST(-1, LEAST(1,
  ...
  INNER JOIN cities c
    ON c.id = uc.city_id
  -- Left join to resolve freight origin city coordinates when origin_city_id is set
  LEFT JOIN cities fc
    ON fc.id = f.origin_city_id
  WHERE f.status = 'OPEN'::freight_status
    AND c.lat IS NOT NULL
    AND c.lng IS NOT NULL
    -- Match by: city_id exact, OR city name exact, OR radius
    AND (
      -- 1. Exact city_id match
      (f.origin_city_id IS NOT NULL AND f.origin_city_id = uc.city_id)
      OR
      -- 2. City name match (fallback when no city_id)
      (LOWER(f.origin_city) = LOWER(c.name) AND LOWER(f.origin_state) = LOWER(c.state))
      OR
      -- 3. Radius match using haversine (max 300km, capped by user's radius_km)
      (
    ...
    -- DEDUPE: exclude items already seen with active exposure
    AND NOT EXISTS (
