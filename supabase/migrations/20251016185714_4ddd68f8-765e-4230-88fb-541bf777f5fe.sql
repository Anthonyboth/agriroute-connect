-- Remover metadados de confirmação incorretos do frete que não foi completado
UPDATE freights
SET metadata = metadata - 'confirmed_by_producer' - 'delivery_confirmed_at' - 'delivery_reported_at',
    updated_at = NOW()
WHERE id = 'c9398688-39ca-43b4-b878-ad3543ec42ce'
  AND accepted_trucks < required_trucks;

-- Atualizar a RPC driver_update_freight_status para permitir transições de OPEN
-- e bloquear mudanças em fretes já confirmados
...
BEGIN
  ...
  IF NOT (
    ...
    (v_freight.status = 'OPEN' AND p_new_status IN ('LOADING', 'ACCEPTED')) OR -- NOVA TRANSIÇÃO
