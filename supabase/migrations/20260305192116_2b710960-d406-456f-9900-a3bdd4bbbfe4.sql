...
-- Fix 1: Migrate all references from wrong city (MA) to correct city (MT), then delete duplicate
UPDATE freights 
SET origin_city_id = '72e2661e-0ffc-4d4f-a032-004edd82a0d8',
    origin_state = 'MT'
WHERE origin_city_id = '1362bc8d-433b-4d63-8e0d-f51c91bb2861';

UPDATE freights 
SET destination_city_id = '72e2661e-0ffc-4d4f-a032-004edd82a0d8',
    destination_state = 'MT'
WHERE destination_city_id = '1362bc8d-433b-4d63-8e0d-f51c91bb2861';

UPDATE user_cities
SET city_id = '72e2661e-0ffc-4d4f-a032-004edd82a0d8'
WHERE city_id = '1362bc8d-433b-4d63-8e0d-f51c91bb2861';

UPDATE driver_availability
SET city_id = '72e2661e-0ffc-4d4f-a032-004edd82a0d8'
WHERE city_id = '1362bc8d-433b-4d63-8e0d-f51c91bb2861';

-- Delete the duplicate city record with wrong state
DELETE FROM cities 
WHERE id = '1362bc8d-433b-4d63-8e0d-f51c91bb2861' AND state = 'MA';

-- Fix 2: Fix get_freights_for_driver RPC - resolve profile_id → auth user_id
...
DECLARE
  v_auth_user_id uuid;
BEGIN
  -- Resolve profile_id → auth user_id for user_cities join
  ...
  SELECT DISTINCT ON (f.id)
    ...
    f.pricing_type::TEXT,
    f.price_per_km,
    f.vehicle_type_required::TEXT,
    f.vehicle_axles_required
  FROM freights f
  INNER JOIN user_cities uc
    ON uc.user_id = v_auth_user_id
  ...
  WHERE f.status = 'OPEN'::freight_status
    ...
    AND NOT EXISTS (
      ...
      WHERE me.viewer_user_id = p_driver_id
