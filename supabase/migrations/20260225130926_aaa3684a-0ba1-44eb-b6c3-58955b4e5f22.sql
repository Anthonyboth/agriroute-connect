...
BEGIN
  ...
  -- ✅ FIX: Selecionar perfil coerente com o painel/role solicitado, evitando pegar
  -- o primeiro perfil legado do usuário (causa feed vazio no painel de motorista).
  SELECT p.id
  INTO v_profile_id
  ...
  WHERE p.user_id = v_auth_uid
    AND (
      ...
      OR upper(coalesce(p.role, '')) = v_role
      OR (
        v_role IN ('MOTORISTA', 'MOTORISTA_AFILIADO')
        AND upper(coalesce(p.role, '')) IN ('MOTORISTA', 'MOTORISTA_AFILIADO')
      )
    )
  ORDER BY
    CASE
      WHEN upper(coalesce(p.active_mode, '')) = v_role THEN 0
      WHEN upper(coalesce(p.role, '')) = v_role THEN 1
      ELSE 2
    END,
    p.created_at ASC
  LIMIT 1;
