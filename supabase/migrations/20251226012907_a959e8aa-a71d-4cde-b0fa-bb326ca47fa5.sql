-- Corrige validação de acesso nas RPCs de relatórios para suportar perfis (profiles.id) vinculados ao usuário (auth.uid)
-- Problema atual: as funções comparam p_profile_id (profiles.id) com auth.uid() (auth.users.id), causando "Acesso negado".
...
BEGIN
  ...
  -- Permite múltiplos perfis por usuário: valida se o perfil pertence ao usuário autenticado
  IF NOT EXISTS (
...
CREATE OR REPLACE FUNCTION public.get_driver_report_charts(p_profile_id uuid, p_start_at timestamp with time zone, p_end_at timestamp with time zone)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.get_producer_report_charts(p_profile_id uuid, p_start_at timestamp with time zone, p_end_at timestamp with time zone)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
-- Ajuste equivalente para COMPANY (transportadora) para comparar via profiles.user_id
...
CREATE OR REPLACE FUNCTION public.get_company_report_charts(p_company_id uuid, p_start_at timestamp with time zone, p_end_at timestamp with time zone)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
