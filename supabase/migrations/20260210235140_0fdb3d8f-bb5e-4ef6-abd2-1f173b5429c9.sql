...
-- Atualizar auto_cancel_overdue_freights para notificar o produtor quando frete é cancelado
...
BEGIN
  ...
  -- =========================================================
  -- FASE 2: CANCELAR FRETES SEM NENHUM ACEITE + NOTIFICAR
  -- =========================================================
  FOR v_freight_record IN
    SELECT f.id, f.service_type, f.created_at, f.pickup_date, f.producer_id,
  ...
  LOOP
    ...
    IF v_real_accepted_count = 0 THEN
      ...
      -- ✅ NOTIFICAR o produtor/criador do frete
      IF v_freight_record.producer_id IS NOT NULL THEN
        INSERT INTO notifications (user_id, title, message, type, data, read)
        VALUES (
          v_freight_record.producer_id,
          '🚫 Frete cancelado automaticamente',
          'Seu frete de ' || COALESCE(v_freight_record.origin_city, '?') || 
            ' → ' || COALESCE(v_freight_record.destination_city, '?') || 
            ' foi cancelado porque nenhum motorista aceitou dentro do prazo.',
          'freight_cancelled',
          jsonb_build_object(
            'freight_id', v_freight_record.id,
            'reason', 'auto_cancel_no_acceptance',
            'service_type', v_freight_record.service_type
          ),
          false
        );
      END IF;
