...
-- Fix: Allow assignment to ACCEPTED multi-truck freights that still have available slots
...
BEGIN
  ...
  -- 4) Verificar se o frete está disponível e tem vagas
  -- CORRIGIDO: Permitir ACCEPTED para fretes multi-carreta que ainda têm vagas
  ...
  -- 8) Se agora o frete está lotado, atualizar status para ACCEPTED (se ainda estava OPEN)
  ...
  ELSE
    -- Frete parcialmente preenchido: manter ACCEPTED se já estava, ou mudar de OPEN para ACCEPTED
    UPDATE freights
    SET status = 'ACCEPTED',
        updated_at = now()
    WHERE id = p_freight_id
