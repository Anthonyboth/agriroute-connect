...
BEGIN
  ...
  -- Fix principal: para PRESTADOR_SERVICOS não depender de tipos urbanos.
  -- O feed deve considerar os tipos do próprio perfil + cidades ativas do usuário.
  IF v_role = 'PRESTADOR_SERVICOS' THEN
    ...
    service_eval AS (
      SELECT
        c.*,
        EXISTS (
          SELECT 1 FROM viewer_types vt WHERE vt.service_type = c.canonical_service_type
        ) AS type_match,
        EXISTS (
          SELECT 1 FROM viewer_cities vc WHERE vc.city_id = c.city_id
        ) AS city_match
      FROM candidates c
    ),
    eligible AS (
    ...
    RETURN jsonb_build_object(
      ...
      'metrics', jsonb_build_object(
        'feed_total_eligible', coalesce(jsonb_array_length(v_items), 0),
  ...
  -- Demais papéis continuam usando a fonte determinística unificada.
...
IS 'Feed determinístico de serviços; fix para PRESTADOR_SERVICOS sem dependência indevida de tipos urbanos e com cast role::text para evitar erro 42804.';