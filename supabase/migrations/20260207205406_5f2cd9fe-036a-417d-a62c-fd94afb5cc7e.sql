      ...
      'charts', json_build_object(
        ...
        'valor_por_dia', (
          SELECT COALESCE(json_agg(d ORDER BY d.dia), '[]'::json) FROM (
            SELECT dia, SUM(valor) as valor FROM (
              ...
              GROUP BY 1
              UNION ALL
              SELECT date_trunc('day', completed_at)::date as dia, COALESCE(SUM(final_price), 0) as valor
              FROM service_request_history 
        ...
        'por_tipo', (
          SELECT COALESCE(json_agg(t), '[]'::json) FROM (
            SELECT name, SUM(value) as value FROM (
              SELECT COALESCE(cargo_type, 'outros') as name, COUNT(*) as value
              FROM freight_history 
              ...
              GROUP BY cargo_type
              UNION ALL
              SELECT 'Serviço' as name, COUNT(*) as value
              FROM service_request_history 
              WHERE client_id = p_profile_id AND completed_at >= p_date_from AND completed_at <= p_date_to
              GROUP BY 1
            ) combined
        ...
        'por_status', (
          SELECT COALESCE(json_agg(s), '[]'::json) FROM (
            SELECT name, SUM(value) as value FROM (
              -- Fretes por status (usando completed_at OU cancelled_at)
        ...
        'ultimas_operacoes', (
          SELECT COALESCE(json_agg(op ORDER BY op.data DESC), '[]'::json) FROM (
            SELECT freight_id, origin_city, origin_state, destination_city, destination_state, 
                   price_total, status_final, completed_at as data, cargo_type, required_trucks,
                   'FRETE' as tipo_operacao
            FROM freight_history 
            WHERE producer_id = p_profile_id 
              AND COALESCE(completed_at, cancelled_at, created_at) >= p_date_from 
              AND COALESCE(completed_at, cancelled_at, created_at) <= p_date_to
            UNION ALL
            SELECT id as freight_id, NULL as origin_city, NULL as origin_state, NULL as destination_city, NULL as destination_state,
                   final_price as price_total, status_final, completed_at as data, service_type as cargo_type, NULL as required_trucks,
      ...
      'kpis', (
        SELECT json_build_object(
          ...
          'avaliacao_media', COALESCE((
            SELECT AVG(dr.rating) FROM driver_ratings dr 
            WHERE dr.driver_id = p_profile_id AND dr.created_at >= p_date_from AND dr.created_at <= p_date_to
          ), 0),
          'total_avaliacoes', (
            SELECT COUNT(*) FROM driver_ratings dr 
            WHERE dr.driver_id = p_profile_id AND dr.created_at >= p_date_from AND dr.created_at <= p_date_to
          ),
