-- Recriar função get_compatible_freights_for_driver sem pricing_type
-- e com correções de tipos e fallback por user_cities
...
BEGIN
  ...
  -- Buscar service_types do motorista
  ...
  city_matches AS (
    ...
    FROM freights f
    JOIN user_cities uc ON uc.user_id = v_user_id 
    ...
    WHERE f.status = 'OPEN'
      ...
      AND (
        ...
        OR (
          ...
          AND (
            (lower(c.name) = lower(COALESCE(f.origin_city, '')) 
             AND lower(c.state) = lower(COALESCE(f.origin_state,'')))
            OR
            (lower(c.name) = lower(COALESCE(f.destination_city,'')) 
             AND lower(c.state) = lower(COALESCE(f.destination_state,'')))
          )
        )
      )
  ),
  ...
  SELECT 
    ...
    CASE 
      WHEN all_matches.match_distance_m IS NOT NULL 
      THEN ROUND(all_matches.match_distance_m / 1000.0, 1) 
      ELSE NULL 
    END AS distance_km,
    f.id AS freight_id,
