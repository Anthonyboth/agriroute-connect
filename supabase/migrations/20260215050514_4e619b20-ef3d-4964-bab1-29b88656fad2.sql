...
-- Fix get_reports_dashboard MOTORISTA panel: Add service_request_history, despesas, avaliações, distância
-- Also fix field name mismatch: km_total -> distancia_total_km
...
BEGIN
  ...
  -- =====================================================
  -- PAINEL MOTORISTA (FIXED: add service_requests, despesas, avaliacoes, distancia)
  -- =====================================================
  WHEN 'MOTORISTA' THEN
    SELECT json_build_object(
      'kpis', (
        SELECT json_build_object(
          -- Receita de fretes rurais
          ...
          -- ✅ NEW: total_avaliacoes (freight_ratings)
          'total_avaliacoes', (
          ...
          -- ✅ FIX: Rename km_total -> distancia_total_km (matches frontend)
          ...
          -- ✅ NEW: Receita de serviços urbanos (GUINCHO, MUDANCA, PET, PACOTES, etc.)
        ...
        'receita_por_mes', (
          SELECT COALESCE(json_agg(d ORDER BY d.mes), '[]'::json) FROM (
            ...
            SELECT mes, SUM(receita) as receita FROM (
              ...
              GROUP BY 1
              UNION ALL
              -- ✅ NEW: Receita de serviços urbanos
              SELECT to_char(srh.completed_at, 'YYYY-MM') as mes,
                     SUM(srh.final_price) as receita
        ...
        'ultimas_operacoes', (
          SELECT COALESCE(json_agg(op ORDER BY op.data DESC), '[]'::json) FROM (
            ...
            WHERE fah.driver_id = p_profile_id 
              ...
              AND COALESCE(fah.completed_at, fah.created_at) <= p_date_to
            UNION ALL
            -- ✅ NEW: Serviços urbanos
            SELECT srh.id, srh.city as origin_city, srh.city as destination_city,
                   srh.final_price as receita,
                   srh.status_final, srh.completed_at as data, srh.service_type as cargo_type, 'SERVICO' as tipo
            FROM service_request_history srh
            WHERE srh.provider_id = p_profile_id
              AND srh.completed_at >= p_date_from AND srh.completed_at <= p_date_to
            ORDER BY data DESC LIMIT 20
