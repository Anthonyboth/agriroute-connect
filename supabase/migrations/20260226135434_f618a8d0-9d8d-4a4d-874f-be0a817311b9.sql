  ...
  IF v_has_urban_types THEN
    ...
    service_eval AS (
      SELECT s.*,
        ...
        EXISTS (
          SELECT 1 FROM svc_viewer_cities vc
          WHERE vc.city_id = s.city_id
             OR (
               s.location_lat IS NOT NULL AND s.location_lng IS NOT NULL
               AND vc.lat IS NOT NULL AND vc.lng IS NOT NULL
               AND 6371 * acos(
                 GREATEST(-1, LEAST(1,
                   cos(radians(s.location_lat)) * cos(radians(vc.lat)) *
                   cos(radians(vc.lng) - radians(s.location_lng)) +
                   sin(radians(s.location_lat)) * sin(radians(vc.lat))
                 ))
               ) <= vc.radius_km
             )
        ) AS city_match,
