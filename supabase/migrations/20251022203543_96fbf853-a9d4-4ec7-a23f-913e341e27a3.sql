-- ============================================================================
-- MIGRATION: Fix Driver Dashboard Critical Errors
-- Issues Fixed:
...
AS $$
  SELECT
    public.is_admin()
    OR EXISTS (
      -- Company owner (company profile belongs to user)
      SELECT 1
...
AS $$
  SELECT
    public.is_admin()
    OR EXISTS (
      -- Only the company owner (company profile belongs to the user)
      SELECT 1
      FROM public.transport_companies tc
      JOIN public.profiles p ON p.id = tc.profile_id
      WHERE tc.id = _company_id AND p.user_id = _user_id
...
-- ============================================================================
-- STEP 2: Drop ALL existing policies on transport_companies (remove recursion)
-- ============================================================================
...
-- ============================================================================
-- STEP 4: Drop ALL existing policies on company_drivers (avoid cross-table subqueries)
-- ============================================================================
...
-- ============================================================================
-- STEP 7: Create driver_checkins table with RLS
-- ============================================================================
...
-- Drivers see their own check-ins; admins see all
...
-- ============================================================================
-- MIGRATION COMPLETE
-- Fixed: RLS recursion, RPC type mismatch, missing checkins table
-- ============================================================================