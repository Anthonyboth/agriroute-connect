-- Fix requires_monitoring column references in RPC functions
-- This column doesn't exist in the freights table and is causing errors

-- Drop and recreate get_compatible_freights_for_driver WITHOUT requires_monitoring
...
-- Drop and recreate get_nearby_freights_for_driver WITHOUT requires_monitoring
DROP FUNCTION IF EXISTS get_nearby_freights_for_driver(uuid, numeric);

CREATE OR REPLACE FUNCTION get_nearby_freights_for_driver(p_driver_id uuid, p_radius_km numeric DEFAULT 100)
RETURNS TABLE(
  freight_id uuid,
  cargo_type text,
  origin_city text,
...
DECLARE
  ...
  driver_lng numeric;
BEGIN
  -- Get driver's current location from their base city
  SELECT uc.lat, uc.lng INTO driver_lat, driver_lng
  FROM user_cities uc
  WHERE uc.profile_id = p_driver_id
    AND uc.is_active = true
    AND uc.city_type = 'MOTORISTA_ORIGEM'
