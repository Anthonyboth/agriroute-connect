...
BEGIN
  ...
  -- Normalizar removendo sufixos _URB e _LOG para match genérico
  SELECT array_agg(DISTINCT regexp_replace(upper(t), '(_URB|_LOG)$', ''))
  ...
  WHERE sr.status = 'OPEN'
    ...
    AND (
      -- Normalizar AMBOS os lados: tipo do serviço e tipo do prestador
      regexp_replace(upper(sr.service_type), '(_URB|_LOG)$', '') = ANY(provider_service_types_normalized)
