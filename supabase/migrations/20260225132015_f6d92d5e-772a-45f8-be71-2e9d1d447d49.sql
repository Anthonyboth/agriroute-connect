-- Fix enum casting bug in authoritative feed
-- Root cause: COALESCE(p.role, '') attempts to coerce '' into enum user_role when role is NULL
-- and causes 22P02 (invalid input value for enum user_role: "")
