-- Comprehensive fix for ST_DWithin errors - standardize geography and double precision radius

-- 1. Fix get_compatible_freights_for_driver
...
-- 4. Fix find_providers_by_location
DROP FUNCTION IF EXISTS public.find_providers_by_location(uuid, numeric, numeric);

CREATE OR REPLACE FUNCTION public.find_providers_by_location(
  request_id uuid, 
  request_lat numeric, 
  request_lng numeric
)
...
-- 6. Fix get_service_requests_in_radius
