...
-- Criar função para obter solicitações de serviço filtradas por região
...
BEGIN
    ...
    INTO provider_cities, provider_states, provider_radius, provider_lat, provider_lng
    FROM profiles p
    ...
    SELECT 
        ...
        sr.service_type,
        CASE 
            WHEN sr.location_address IS NOT NULL THEN sr.location_address
        ...
        sr.urgency,
        CASE 
            WHEN sr.contact_phone IS NOT NULL THEN sr.contact_phone
    ...
    SELECT 
        ...
        gr.service_type,
        CASE 
            WHEN gr.payload->>'origin_address' IS NOT NULL THEN gr.payload->>'origin_address'
            ELSE 'Endereço disponível após aceitar'
        END as location_address,
        CASE 
            WHEN gr.payload->>'origin_address' IS NOT NULL THEN gr.payload->>'origin_address'
            ELSE 'Endereço disponível após aceitar'
        END as location_address_safe,
        gr.payload->>'problem_description' as problem_description,
        ...
        END as urgency,
        CASE 
            WHEN gr.contact_phone IS NOT NULL THEN gr.contact_phone
            ELSE 'Telefone disponível após aceitar'
        END as contact_phone,
        CASE 
            WHEN gr.contact_phone IS NOT NULL THEN gr.contact_phone
        ...
        (gr.payload->>'pricing'->>'estimated_price')::numeric as estimated_price,
        gr.status,
        gr.created_at,
        CASE 
