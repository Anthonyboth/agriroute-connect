...
const ProducerDashboard = () => {
  ...
  const statistics = useMemo(() => {
    return {
      ...
      activeFreights: freights.filter(f => ['IN_NEGOTIATION', 'ACCEPTED', 'IN_TRANSIT'].includes(f.status) && f.status !== 'DELIVERED').length,
      ...
      <div className="container max-w-7xl mx-auto py-4 px-4">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="ongoing" className="space-y-4">
            ...
            {freights.filter(f => ['IN_NEGOTIATION', 'ACCEPTED', 'IN_TRANSIT'].includes(f.status) && f.status !== 'DELIVERED').length === 0 ? (
              <Card className="border-dashed">
                <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <div className="grid gap-6 md:grid-cols-1 xl:grid-cols-2 2xl:grid-cols-3">{freights.filter(f => ['IN_NEGOTIATION', 'ACCEPTED', 'IN_TRANSIT'].includes(f.status) && f.status !== 'DELIVERED').map((freight) => (
