// Proposal domain components - barrel export
export { FreightProposalsManager } from '../FreightProposalsManager';
export { ProposalCounterModal } from '../ProposalCounterModal';
export { ProposalModal } from '../ProposalModal';
export { ServiceProposalModal } from '../ServiceProposalModal';
export { FlexibleProposalModal } from '../FlexibleProposalModal';
export { DriverProposalDetailsModal } from '../DriverProposalDetailsModal';
