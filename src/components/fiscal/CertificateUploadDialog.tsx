...
export function CertificateUploadDialog({ open, onOpenChange, onSuccess }: CertificateUploadDialogProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // ✅ SE FALHOU, MOSTRAR O ERRO REAL (hookError)
      if (!success) {
        const realMsg =
          (hookError && hookError.trim().length > 0 ? hookError : null) || "Não foi possível enviar o certificado";

        // IMPORTANTE: mostrar detalhes e também logar pra debug
        console.error("[CERTIFICATE UPLOAD] Falhou:", { hookError: realMsg });
      ...
      // ✅ SE ESTOURAR EXCEPTION, TAMBÉM MOSTRAR DETALHES
      const msg =
        (e?.message && String(e.message).trim().length > 0 ? String(e.message) : null) ||
        (hookError && hookError.trim().length > 0 ? hookError : null) ||
        "Não foi possível enviar o certificado";

      console.error("[CERTIFICATE UPLOAD] Exception:", e);
      toast.error(msg);
    } finally {
      setUploading(false);
    }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        {uploadSuccess ? (
          <div className="py-8 text-center">
            ...
            <p className="text-muted-foreground text-sm mt-2">Aguarde a validação automática...</p>
          </div>
        ) : (
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label>Arquivo do Certificado *</Label>
              <Input type="file" accept=".pfx,.p12" onChange={handleFileSelect} ref={fileInputRef} className="hidden" />
              <div
                className={`
