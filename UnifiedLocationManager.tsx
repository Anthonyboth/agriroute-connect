...
const UnifiedLocationManager: React.FC<UnifiedLocationManagerProps> = ({ userType, onAreasUpdate }) => {
  ...
  // Função para buscar coordenadas automaticamente baseado no nome da cidade (com wrapper seguro)
  const geocodeCity = async (cityName: string, state?: string) => {
    if (!cityName.trim()) return;
    
    try {
      const result = await safeNominatimGeocode(cityName, state, 'Brasil');
      if (result) {
        setFormData(prev => ({
          ...prev,
          lat: result.latitude,
        ...
        toast.error('Não foi possível encontrar coordenadas para a cidade informada');
      }
    } catch (error) {
